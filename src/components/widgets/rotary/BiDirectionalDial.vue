...
const handleChange = (event: MouseEvent) => {
  ...
  if (angle.value >= minDegree && angle.value <= maxDegree) {
    let angleValue = angle.value - event.movementY
    if(angleValue < minDegree) {
      angleValue = minDegree
    } else if(angleValue > maxDegree) {
      angleValue = maxDegree
    }
    angle.value = angleValue
  }
  let max = props.widget.midiData.messageMax
  let positiveAngleValue = angle.value + maxDegree
  update(Math.round((max / totalDegrees) * positiveAngleValue))
}
...
const handleReset = () => {
  update(parseFloat(((props.widget.midiData.messageMax) / 2).toFixed(1)))
  angle.value = 0
}
...
const drawUpdateArc = (newValue: number) => {
  ...
  foregroundCtx.value.arc(39, 25, 23, 270*Math.PI/180, minAngleRadian + endAngle, angle.value < 0);
